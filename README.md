...
To show a basic dialog that informs the user that a compatible wallet needs to be installed can be done with following method. Notice that the dialog generated by this method is basic and provide the minimum required information. The creation of a new dialog is advised, to be compatible with the layout of your application.
...
```
If you follow with the use of the code above the resources bellow are required.
```
...
Finally, the following method is the one used to redirect the used to AppCoins Wallet store page. 
```
